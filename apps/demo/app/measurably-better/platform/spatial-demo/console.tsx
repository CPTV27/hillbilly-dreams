...
// ─────────────────────────────────────────────────────────────
// SPX Console — rebranded from S2PX
// Spatial Intelligence Platform — Hillbilly Dreams, Inc.
// ─────────────────────────────────────────────────────────────
const C = {
  bg: '#ffffff',
  bgGradient: '#f8f9fa',
...
// ─────────────────────────────────────────────────────────────
// Platform Parameters — SPX baseline scenario
// ─────────────────────────────────────────────────────────────
const JV = {
  coreNode: 5_000,
  growthModule: 1_000,
      ...
      <div style={{ minHeight: '100vh', background: C.bgGradient, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, fontFamily: '"Google Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
        <div style={{ maxWidth: 620, width: '100%', textAlign: 'center', animation: 'slideUp 0.5s cubic-bezier(0.16, 1, 0.3, 1)' }}>
          ...
          <h1 style={{ fontSize: 44, fontWeight: 400, color: C.text, margin: '0 0 16px', letterSpacing: '-0.03em' }}>
            SPX Operator Console
          </h1>
          <p style={{ fontSize: 22, color: C.textSecondary, margin: '0 0 48px', lineHeight: 1.6 }}>
            Let&apos;s configure it for your operation. Takes about 30 seconds.
          </p>
...
// ─────────────────────────────────────────────────────────────
const SUGGESTED = [
  'What does the $5,000/month Core Node actually get me?',
  'How does the commission buyout work?',
  'What happens if we process 2,500 scans next year?',
  'Show me the break-even math',
  'What is the risk if automation only hits 15%?',
];
...
function ConsoleApp({ interests }: { interests: string[] }) {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    const context = {
      ...
      instructions: `You are advising the operator of an SPX-licensed spatial intelligence business on this technology platform proposal. Be direct, use real numbers from the scenario context, and answer like a CFO would — no fluff. The user cares most about: ${interests.map(id => CAPABILITIES.find(c => c.id === id)?.title).filter(Boolean).join(', ')}. Emphasize these areas. Keep formatting clean and readable.`,
    };
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <span style={{ fontWeight: 600, color: C.text, fontSize: 24, letterSpacing: '-0.02em' }}>SPX</span>
          <span style={{ color: C.textTertiary, fontSize: 18, fontWeight: 400 }}>Spatial Intelligence Console</span>
        </div>
              ...
              <div key={i} style={{
                ...
                {msg.role === 'ai' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, marginLeft: 6 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: C.textTertiary, letterSpacing: '0.03em' }}>SPX AI</span>
                  </div>
                )}
                <div style={{
                  maxWidth: '85%',
