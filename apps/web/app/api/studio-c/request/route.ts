...
const VALID_TYPES = ['session-recording', 'promo-video', 'property-tour', 'social-clips', 'bts', 'documentary'];
  ...
  try {
    ...
    const req = await prisma.studioCRequest.create({
      data: {
        ...
        requestType,
        brief: brief.trim(),
        location: location || 'Utopia Studios, Bearsville NY',
        preferredDate: preferredDate ? new Date(preferredDate) : null,
        budget: budget || 'quote-requested',
        status: 'submitted',
      },
    });
    ...
    console.error('[POST /api/studio-c/request]', err);
