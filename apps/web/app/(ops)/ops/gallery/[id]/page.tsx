        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '48rem' }}>
            ...
            <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                    <h1 style={{ fontSize: '2rem', fontWeight: 700, margin: 0, color: 'var(--theme-text-primary)' }}>Review Artist: {app.name}</h1>
                    <span style={{
            ...
            {/* Application Data */}
            <div style={{ backgroundColor: 'var(--theme-card-bg)', border: '1px solid var(--theme-card-border)', borderRadius: '1rem', overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                        <div>
                            ...
                            <div style={{ color: 'var(--theme-text-primary)', fontWeight: 500 }}><a href={`mailto:${app.email}`} style={{color:'var(--theme-accent)'}}>{app.email}</a></div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--theme-text-muted)', marginBottom: '0.25rem' }}>Location</div>
                            <div style={{ color: 'var(--theme-text-primary)', fontWeight: 500 }}>{app.city}, {app.state}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--theme-text-muted)', marginBottom: '0.25rem' }}>Medium</div>
                            <div style={{ color: 'var(--theme-text-primary)', fontWeight: 500 }}>{app.medium}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--theme-text-muted)', marginBottom: '0.25rem' }}>Years Active</div>
                            <div style={{ color: 'var(--theme-text-primary)', fontWeight: 500 }}>{app.yearsActive || 'N/A'}</div>
                        </div>
                    </div>

                    <div style={{ borderTop: '1px solid var(--theme-card-border)', paddingTop: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                         <div>
                            ...
                            <div style={{ color: 'var(--theme-text-primary)', fontWeight: 500 }}><a href={app.portfolioUrl.startsWith('http') ? app.portfolioUrl : `https://${app.portfolioUrl}`} target="_blank" rel="noreferrer" style={{color:'var(--theme-accent)'}}>{app.portfolioUrl}</a></div>
                        </div>
                        <div>
                            <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--theme-text-muted)', marginBottom: '0.25rem' }}>Prices / Instagram</div>
                            <div style={{ color: 'var(--theme-text-primary)', fontWeight: 500 }}>
                                {app.priceRange || 'N/A'} {app.instagram && <span style={{color: 'var(--theme-text-muted)', marginLeft: '1rem'}}>IG: {app.instagram}</span>}
                            </div>
                        </div>
                    </div>

                    <div style={{ borderTop: '1px solid var(--theme-card-border)', paddingTop: '1.5rem' }}>
                        <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--theme-text-muted)', marginBottom: '0.5rem' }}>Artist Bio</div>
                    ...
                    {app.whyBCA && (
                        <div style={{ borderTop: '1px solid var(--theme-card-border)', paddingTop: '1.5rem' }}>
                            <div style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--theme-text-muted)', marginBottom: '0.5rem' }}>Why BCA?</div>
                            <div style={{ color: 'var(--theme-text-primary)', lineHeight: 1.6, whiteSpace: 'pre-wrap', fontStyle: 'italic', paddingLeft: '1rem', borderLeft: '3px solid var(--theme-accent)' }}>
                                "{app.whyBCA}"
                            </div>
                        </div>
                    )}
                ...
                {isPending && (
                    <div style={{ padding: '1.5rem', backgroundColor: 'var(--theme-hover)', borderTop: '1px solid var(--theme-card-border)', display: 'flex', gap: '1rem', alignItems: 'center' }}>
                        <form action={`/api/gallery/applications/${app.id}/approve`} method="POST" style={{ flex: 1 }}>
                            <button type="submit" style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem 1.5rem', fontSize: '0.875rem', fontWeight: 600, color: '#fff', backgroundColor: 'var(--theme-success)', borderRadius: '0.5rem', border: 'none', cursor: 'pointer', transition: 'opacity 0.2s' }}>
                                {iconCheck} Approve & Create Stripe Connect Acct
                            </button>
                        </form>
                        <form action={`/api/gallery/applications/${app.id}/reject`} method="POST">
                            <button type="submit" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem 1.5rem', fontSize: '0.875rem', fontWeight: 600, color: 'var(--theme-text-primary)', backgroundColor: 'var(--theme-card-bg)', borderRadius: '0.5rem', border: '1px solid var(--theme-card-border)', cursor: 'pointer', transition: 'background-color 0.2s' }}>
                                {iconX} Reject
                            </button>
                        </form>
                    </div>
