...
export default function TaskCard({ task }: { task: LaunchTask & { contentPack: ContentPack | null } }) {
    ...
    return (
        <div className={`bg-white border rounded-xl overflow-hidden transition-all shadow-sm ${status === 'done' ? 'border-green-300 bg-green-50/10' : 'border-neutral-200'}`}>
            <div className="p-4 sm:p-6 flex items-start gap-4">
                ...
                <div className="flex-1 space-y-2">
                    <div className="flex flex-wrap items-center gap-2 justify-between">
                        <h3 className={`font-semibold text-lg ${status === 'done' ? 'text-neutral-500 line-through' : 'text-neutral-900'}`}>
            ...
            {expanded && task.guide && (
                <div className="px-4 sm:px-6 py-5 bg-neutral-50 border-t border-neutral-100 text-neutral-700 prose prose-amber max-w-none text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: task.guide }} />
            )}
