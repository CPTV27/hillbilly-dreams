    ...
    <div style={{ minHeight: '100vh', backgroundColor: C.bg, fontFamily: 'JetBrains Mono, SF Mono, Consolas, monospace', color: C.text, fontSize: 13 }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, padding: '12px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ color: C.textDisabled, fontSize: 11 }}>v2.1.0 · us-east4</span>
        </div>
        <div style={{ display: 'flex', gap: 32 }}>
          <Readout label="Margin Recovery" value={`$${marginRecovered.toLocaleString()}`} color={C.success} />
          <Readout label="Capacity Unlocked" value={`${capacityUnlocked} jobs`} color={C.success} />
          <Readout label="Context Bleed Rate" value="0.00%" color={C.success} />
          <Readout label="Year 1 Total" value={`$${(JV.year1Total / 1000).toFixed(0)}K`} color={C.accentHover} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 280px', gridTemplateRows: '1fr auto', height: 'calc(100vh - 49px)' }}>
        ...
        <div style={{ borderRight: `1px solid ${C.border}`, padding: 24, display: 'flex', flexDirection: 'column', gap: 32, overflowY: 'auto' }}>
          <div>
            ...
            <label style={{ display: 'block', marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: C.textDisabled, marginTop: 4 }}>
            ...
            <button onClick={handleUpdateSimulation}
              style={{ width: '100%', padding: '10px', borderRadius: 8, backgroundColor: C.border, border: `1px solid ${C.surfaceHi}`, color: C.accentHover, fontWeight: 700, cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
              ▶ Run Simulation
            </button>
          </div>

          <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 24 }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.textMuted, marginBottom: 16 }}>JV Financial Model</div>
            ...
            <div style={{ borderTop: `1px solid ${C.surfaceHi}`, marginTop: 12, paddingTop: 12 }}>
              <MetricRow label="Projected Revenue" value={`$${(projectedRevenue / 1000).toFixed(0)}K`} color={C.success} />
              <MetricRow label="Projected Net" value={`$${(projectedNet / 1000).toFixed(0)}K`} color={C.success} />
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: 24, overflowY: 'auto' }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.textMuted, marginBottom: 8 }}>Architecture Comparison</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr 1fr', gap: 2 }}>
              <div style={{ padding: '12px 16px', backgroundColor: C.surface, borderRadius: '8px 0 0 0' }}>
                <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.textMuted }}>Dimension</span>
              </div>
            ...
            {MATRIX_ROWS.map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '160px 1fr 1fr', gap: 2 }}>
                <div style={{ padding: '14px 16px', backgroundColor: C.surface, display: 'flex', alignItems: 'center' }}>
                  <span style={{ fontWeight: 700, color: C.text, fontSize: 12 }}>{row.dimension}</span>
                </div>
                <div style={{ padding: '14px 16px', backgroundColor: 'rgba(239,68,68,0.04)', borderLeft: `2px solid ${C.error}20` }}>
                  <span style={{ color: C.textSecondary, fontSize: 12, lineHeight: 1.5 }}>{row.consumer}</span>
                </div>
                <div style={{ padding: '14px 16px', backgroundColor: 'rgba(34,197,94,0.04)', borderLeft: `2px solid ${C.success}40` }}>
                  <span style={{ color: C.text, fontSize: 12, lineHeight: 1.5 }}>{row.sovereign}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ borderLeft: `1px solid ${C.border}`, padding: 24, display: 'flex', flexDirection: 'column', gap: 24, overflowY: 'auto' }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.textMuted, marginBottom: 16 }}>Context Packager</div>
              ...
              {[
                ...
                <label key={ds.key} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 8, backgroundColor: datasets[ds.key] ? 'rgba(14,165,233,0.08)' : C.surface, border: `1px solid ${datasets[ds.key] ? C.accentHover + '40' : C.border}`, cursor: 'pointer' }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: C.textDisabled, fontSize: 10 }}>{ds.count}</div>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: C.textMuted, marginBottom: 12 }}>Isolation Protocol</div>
            <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 8, backgroundColor: antiPollution ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)', border: `1px solid ${antiPollution ? C.success + '40' : C.error + '40'}`, cursor: 'pointer' }}>
              <div>
                <div style={{ color: C.white, fontWeight: 700, fontSize: 12 }}>Anti-Pollution Override</div>
                <div style={{ color: antiPollution ? C.success : C.error, fontSize: 10, fontWeight: 600 }}>
              ...
              <div onClick={() => setAntiPollution(!antiPollution)}
                style={{ width: 44, height: 24, borderRadius: 12, backgroundColor: antiPollution ? C.success : C.surfaceHi, position: 'relative', cursor: 'pointer', transition: 'background 0.2s' }}>
            ...
            <button onClick={handleInjectContext}
              style={{ width: '100%', marginTop: 12, padding: '10px', borderRadius: 8, backgroundColor: C.border, border: `1px solid ${C.surfaceHi}`, color: C.success, fontWeight: 700, cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
        ...
        <div style={{ gridColumn: '1 / -1', borderTop: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', maxHeight: 240 }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 24px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {chatLog.map((msg, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <span style={{ fontWeight: 700, fontSize: 11, color: msg.role === 'system' ? C.textDisabled : msg.role === 'user' ? C.accentHover : C.success, minWidth: 60, flexShrink: 0 }}>
                  {msg.role === 'system' ? '[SYS]' : msg.role === 'user' ? '[EXEC]' : '[ENGINE]'}
                </span>
                <span style={{ color: msg.role === 'system' ? C.textDisabled : C.text, lineHeight: 1.5 }}>{msg.text}</span>
              </div>
            ))}
          </div>
          <form onSubmit={handleChat} style={{ display: 'flex', borderTop: `1px solid ${C.border}`, padding: '8px 24px', gap: 8 }}>
            <span style={{ color: C.success, fontWeight: 700, padding: '8px 0', fontSize: 12 }}>▸</span>
            <input
              value={chatInput}
            ...
            />
            <button type="submit" style={{ backgroundColor: C.border, border: `1px solid ${C.surfaceHi}`, color: C.accentHover, padding: '6px 16px', borderRadius: 6, fontWeight: 700, cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
              Execute
            </button>
          </form>
        </div>
...
function MetricRow({ label, value, color, highlight }: { label: string; value: string; color?: string; highlight?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${C.border}30` }}>
      <span style={{ color: highlight ? C.error : C.textMuted, fontSize: 12 }}>{label}</span>
      <span style={{ color: color || C.text, fontWeight: 600, fontSize: 12 }}>{value}</span>
    </div>
  );
}
