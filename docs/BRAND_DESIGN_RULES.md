...
The existing infrastructure (`lib/imagen.ts`, `/api/media/generate`, GCS upload) just needs the brand context injected into the generation prompt. Delta Dawn should prepend the brand rules to every Imagen prompt.
