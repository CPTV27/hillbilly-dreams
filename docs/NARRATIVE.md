# THE NARRATIVE — Single Source of Truth
## Last Updated: March 25, 2026
## RULE: Every page, email, deck, and agent references this document. If the narrative changes, update here FIRST.

---

## THE IDEA

Measurably Better Things is not a SaaS company that happens to be in Mississippi. It's an economic engine that uses technology to keep money circulating locally. The products are the visible surface. The local economy is the point.

Every product in the lineup is an expression of one idea: **build the infrastructure locally, run it locally, keep the value locally.** The technology is the means. The community becoming measurably better is the end.

---

## THE ONE LINE (brand level — for websites, decks, investors)
"The South has the culture. We're building the infrastructure to match."

## THE TAGLINE (brand level)
"Delivering the efficiencies of tomorrow, today."

## WHAT "MEASURABLY BETTER THINGS" MEANS
Not "our product is measurably better than theirs." It means **the things in your community become measurably better** when the technology infrastructure is owned and operated by people who live there.

---

## THE STREET PITCH (what you say in the shop)

### The Hook
**"It does all the cool stuff you don't know how to do."**

### The Demo
**"Take a picture. Watch what happens."**

You snap a photo. The tool:
1. Writes the caption
2. Picks the hashtags
3. Posts to Instagram, Facebook, Google Business
4. Adds it to your website gallery
5. Tags it for your next email newsletter
6. Logs it as content for the tourism board

One photo. Six outputs. You did one thing. The platform did the rest.

### The Universal Input
**It's just a text box. Ask it anything.**

"Post this photo to all my socials."
"How much did I make last week?"
"Write me a lunch special for tomorrow."
"What are my competitors charging?"
"Make me a video from that photo I took yesterday."
"Remind me to renew my business license in March."
"Draft a response to that bad review."
"What should I charge for catering a 50-person wedding?"

No menus. No tabs. No navigation. No app to learn. Just text it — photos, videos, questions — like you're texting a friend who's really good at everything.

You text it a photo → it posts to all your socials and texts you back the caption it used.
You text it a video → it makes a short and texts you back the link.
You text it a question → it answers with your real numbers.

It's iMessage with your business brain on the other end.

### The Close
**"$20 a month."**

### The Three-Line Version (memorize this)
1. "It does all the cool stuff you don't know how to do."
2. "Take a picture. Watch what happens."
3. "$20 a month."

---

## WHY NOT "READS YOUR BOOKS"
"Same price as ChatGPT, except this one reads your books" was clever but wrong:
- Nobody under 50 calls their accounting "books"
- It frames the product as a bookkeeping tool — it's not
- It undersells the product — it does marketing, CRM, research, social, reputation, everything
- Clever loses to clear when selling to busy people

The product does ALL the stuff the owner doesn't know how to do:
- Marketing? It handles that.
- Bookkeeping? It watches your numbers.
- Reviews? It monitors them.
- Competition? It tracks them.
- Social media? Take a photo, it does the rest.
- You just run your business. It does the rest.

---

## THE PITCH HIERARCHY (from street to boardroom)

### In someone's shop (30 seconds)
"It does all the cool stuff you don't know how to do. Take a picture, watch what happens. $20 a month."

### To a business owner sitting down (2 minutes)
"You know all that stuff you wish you had time for? Marketing, keeping up with reviews, knowing if you're actually making money, figuring out what your competitors are doing? This does all of it. You take a picture, it posts it everywhere. You connect your QuickBooks, it watches your numbers. You ask it a question, it answers with your actual data. $20 a month. Free for a week."

### To a mayor (2 minutes)
"You're going to spend ARPA money on somebody's software by December. We're the only option built in Mississippi. We deploy in 90 days. And every business on Main Street gets a free dashboard."

### To an investor (2 minutes)
"Hillbilly Dreams owns the IP behind a regional technology platform deployed across SMB, civic, education, and media in the Deep South. Big Muddy is the proof of concept. Measurably Better is the revenue engine. The $20 self-serve tier is the distribution vehicle."

### OLD LINE (deprecated — do not use)
~~"Same price as ChatGPT. Except this one reads your books."~~

---

## THE FOUR LEVELS

### Level 1: HILLBILLY DREAMS, INC. (Holding Company)
- Owns all IP, platform, equity
- Not customer-facing. Investors, partners, legal only.
- "Technology infrastructure for the New South."
- Domain: hillbillydreamsinc.com

### Level 2: MEASURABLY BETTER (Product Brand — what we sell)
- The technology platform. The thing buyers interact with.
- Who sees it: SMB operators, city managers, school districts, CVBs.
- "Delivering the efficiencies of tomorrow, today."
- "A regional technology and media provider for the Deep South. We bring enterprise-grade AI infrastructure to Main Street businesses, city halls, school districts, and tourism bureaus — deployed in 30 days, funded by dollars you already have, operated by people who live here."
- Domains: measurablybetterthings.com, measurablybetter.life

**Products (each with its own identity, all running on the same engine):**

| Product | Market | What it does |
|---------|--------|-------------|
| Deep South Directory | Main Street SMB | The front door for local businesses. Free listing → $20 dashboard → $99 automation. DeepSouthDirectory.com |
| S2PX | AEC / Scanning | Spatial intelligence for the built environment |
| Municipal product | City government | City hall front door — website, 311, payments, records search. $9,600-$18K/yr |
| K-12 product | School districts | District dashboard, AI literacy, safety docs. $25K+/yr |

**Platform capabilities (features, not products):**
- Kiosk mode — device lockdown toggle. Any deployment can flip into kiosk mode.
- AI business intelligence — Gemini Pro on Vertex AI against real financial data
- Multi-tenant routing — one codebase, unlimited brands
- Payment rails — Stripe Connect for split payments
- Content engine — CMS for articles, events, playlists, newsletters

### Level 3: BIG MUDDY (Media & Hospitality — the proof of concept)
- The consumer-facing brand. The content engine. The proof that the model works.
- Who sees it: Travelers, music fans, readers, guests.
- "The Mississippi's music corridor. Stay. Drive. Read. Listen."
- Domain: bigmuddytouring.com
- Delta Dawn — the Big Muddy AI agent

| Brand | What it does | One-liner | Lead |
|-------|-------------|-----------|------|
| Touring | The corridor experience | "Memphis to New Orleans and 14 more cities" | |
| Magazine | Editorial content | "Stories from the corridor" | Tracy |
| Radio | Curated playlists & live | "The sound of the Delta" | |
| Records | Artist services | "100% masters retained. Always." | JP |
| The Inn | Natchez property | "The anchor. Where the story started." | Tracy |
| Rise Up | Talent development | "The talent has always been here" | Amy (lead vocalist) |

**Big Muddy Publishing** (Tracy Alderson-Allen — Division Head)
- Big Muddy Magazine (city guides, interviews, photo essays from the corridor)
- Books (Outsider Economics)
- Contributing writers program (three tiers: community/paid/rev-share)
- "Content compounds. Nothing is disposable."

**Big Muddy Hospitality** (Tracy Alderson-Allen — Division Head)
- The Big Muddy Inn (6 rooms, Natchez)
- Weddings & Events
- NOT expanding into more rooms. The Inn is the anchor, the stage, the platform.
- "The rooms fund the ecosystem. Tracy runs the profitable base."
- Needs its own interface and tools

**Big Muddy Entertainment** (JP — Division Head)
- Records, Radio, Touring, Rise Up
- JP has full creative authority. He decides what the department needs.
- "If he wants to produce a TV show, Big Muddy sponsors it."
- Needs its own interface and tools

**Chase Pierson** — Executive Producer
- Oversees the entire operation
- Technology, strategy, product, partnerships
- Not running any single division — running all of them

**Amy Allen**
- ON-NETWORK TALENT. Not operations staff.
- Lead vocalist for Rise Up
- The platform handles the admin work so the people can do what they're actually good at.

### Level 4: OUTSIDER ECONOMICS (Thought Leadership)
- The philosophical foundation. The book. The why.
- Who sees it: Thinkers, builders, policy people.
- "What happens when the economy works for the people who actually live here."
- Domain: outsidereconomics.com

---

## THE FOUR PRINCIPLES (Outsider Economics — Deep South Edition)

1. **Sovereignty** — Own your data. Own your tools. Own your story.
2. **Density** — One platform runs everything. Every new node makes the network smarter.
3. **Compounding** — Every dollar creates a permanent asset. Nothing is disposable.
4. **Local Velocity** — Technology dollars stay in-region and recirculate.

---

## DEEP SOUTH DIRECTORY — THE MAIN STREET PLAY

Deep South Directory is how Measurably Better reaches small businesses. A business owner doesn't sign up for "Measurably Better." They join the Deep South Directory. The listing comes with the dashboard.

**The flow:**
1. Business joins Deep South Directory (free listing — public profile, 100 AI queries, competitive snapshot)
2. They see their data and think "I want more"
3. They upgrade to $20/mo — connects QuickBooks, daily briefings, unlimited AI, guided learning path
4. They outgrow that and upgrade to $99/mo — all sources, marketing automation, alerts, 3 team seats

**Three distribution channels (no ad spend):**
1. **Municipality sponsors businesses** — city buys a block of licenses, funded by ARPA/CDBG. Instant trust, zero sales friction.
2. **Boots on the ground** — "I run the hotel down the street. This is what I use. Want to try it for free?"
3. **CVB / Chamber as distributor** — directory listing bundled with chamber membership.

**Pricing (self-serve):**

| Tier | Price | What they get |
|------|-------|--------------|
| Free | $0 | Google Business Profile analysis, competitive snapshot, 100 AI queries/mo, directory listing, intro training |
| Replace ChatGPT | $20/mo | 7-day free trial. Connect 1 data source. Unlimited AI. Daily briefing. Review monitoring. Monthly P&L. Guided learning path. |
| The Engine | $99/mo | Connect ALL sources. Multi-source correlation. Marketing automation. Real-time alerts. 3 seats. Weekly strategy memo. |
| The Operator | $1,500/mo | Full outsourced operating system. 10 seats. Dedicated AI agent. Revenue forecasting. Phone support. Replaces back-office headcount. |
| Institutional | Custom | Cities, schools, CVBs. Tyler charges $25K-75K/yr. We start at $9,600. Grant-funded. |

---

## THE PITCH LADDER

- **To a Main Street business owner:** "You're on the Deep South Directory. Your listing comes with a free dashboard. Upgrade to $20 and it reads your books."
- **To a mayor:** "Measurably Better digitizes your city. We already run a business on Main Street in Natchez — here's what we built."
- **To a CVB director:** "Big Muddy proves the model — a magazine, a radio station, and a touring route running on one engine. We'll do the same thing with your tourism budget, and keep the money local."
- **To an investor:** "Hillbilly Dreams owns the IP behind a regional technology platform deployed across SMB, civic, education, and media in the Deep South. Big Muddy is the proof of concept. Measurably Better is the revenue engine."
- **To a reader:** "Outsider Economics is the book about why this matters."

---

## THE CROSS-PROMOTION ENGINE

One artist booking triggers seven revenue events:
1. They play the Inn's stage
2. Show gets recorded for Radio
3. Magazine writes the city guide for the tour stop
4. Artist goes on Rise Up
5. Catalog lives on Records
6. Touring route adds the venue
7. Kiosk mode sells the ticket at the front desk

"Every dollar circulates. No division is an island."

---

## THE COMPETITIVE POSITION

| Us | Them |
|----|------|
| Built in Mississippi | Built in Plano, TX or San Francisco |
| Transparent pricing | Opaque, quote-only |
| 90-day deployment | 6-18 month projects |
| AI-first (Gemini Pro) | AI as add-on |
| One engine, four markets | One product, one market |
| Phone call to close | $50K CAC |
| Money stays local | Money leaves the region |

---

## THE NATCHEZ PROOF STACK

| What | Status |
|------|--------|
| The Big Muddy Inn | Live — bookable |
| Big Muddy Magazine | Live — readable |
| Big Muddy Radio | Live — listenable |
| Big Muddy Touring | Live — 18-city corridor |
| Big Muddy Records | Live — artists signed |
| Rise Up | Live — performing |
| Deep South Directory | Building — DeepSouthDirectory.com |
| Measurably Better (SMB) | Live — platform deployed |
| Measurably Better (Civic) | Pilot — Natchez target |
| Measurably Better (Education) | Proposed — Natchez-Adams |

"When we walk into a city council meeting, we don't show slides. We show a hotel you can book, a magazine you can read, and a radio station you can hear."

---

## THE RISE UP MODEL

- Franchisable touring brand (like Yachtley Crew / Blue Man Group)
- Multiple units, multiple regions, eventually multiple continents
- Amy + Arri anchor the flagship
- Regional talent fills rotating cast from tour stop talent searches
- Every show is a talent search AND a community investment
- Europe touring planned after US proof of concept

---

## THE CIVIC OPPORTUNITY

- Mississippi: ~$826M unspent ARPA, deadline Dec 2026 (9 months)
- E-Rate: $33.3M in FY2026 MS requests, new 5-year Category 2 cycle
- Title IV-A: $17.4M/year for MS K-12 technology
- Tyler Technologies minimum: $25-75K/year. We come in at $9,600-18K.
- VERIFIED GAP: No regional provider serves all four segments in the Deep South.

---

## VOICE RULES

- Direct but warm. Say what you mean, then stop.
- Concrete details over abstract claims.
- Never say: synergy, leverage, ecosystem (in marketing), game-changer, disruptive
- The South is not a victim. It's underleveraged. Frame everything as forward motion.
- "We're not fixing what's broken. We're building what was never built."

---

## KEY STATS FOR ANY PITCH

- $19.7B U.S. live music market (2026)
- 82% of independent artists can't afford to tour
- $826M unspent ARPA in Mississippi (Dec 2026 deadline)
- $33.3M E-Rate requests filed for MS FY2026
- Tyler Technologies: $2.3B revenue, 89% recurring — the benchmark
- CivicPlus: 10,000+ municipalities, 33.6% market share
- 144 school districts in Mississippi, most under 5,000 students
