# Delta Dawn — Canonical Tool Schemas (Grok-Validated)

*April 6, 2026. Reviewed by Grok. 10 tools, JSON schemas, guardrails locked.*

---

## Tool Count: 10 (Sweet Spot for Gemini 2.5 Flash)

Do not add more without consolidating. 8-15 is optimal. Beyond 15, hallucination rate increases.

---

## Tool Definitions

### 1. query_database
NL2SQL. Natural language → parameterized SQL → Neon → JSON. Read-only.

```json
{
  "name": "query_database",
  "description": "Query the HDI PostgreSQL database using natural language. Converts to parameterized SQL, executes read-only, returns JSON results. Always filtered by tenant. Max 500 rows. 30-second timeout.",
  "parameters": {
    "type": "object",
    "properties": {
      "query": { "type": "string", "description": "Natural language query about business data" },
      "tenantId": { "type": "string", "description": "Tenant filter. Auto-filled from user context." }
    },
    "required": ["query"]
  }
}
```

**Guardrails (mandatory):**
- Read-only Postgres role (no INSERT/UPDATE/DELETE/DROP)
- Parameterized queries only (no string interpolation)
- Tenant filter enforced on every query
- 30-second timeout
- Max 500 rows returned
- Block DDL/DML keywords: DROP, DELETE, UPDATE, INSERT, ALTER, TRUNCATE, CREATE
- Query logged to OpsActivity

### 2. get_schema
Prisma schema introspection by domain.

```json
{
  "name": "get_schema",
  "description": "Returns the Prisma schema models for a specific business domain. Use before query_database to understand available tables and fields.",
  "parameters": {
    "type": "object",
    "properties": {
      "domain": { "type": "string", "enum": ["directory", "revenue", "content", "events", "artists", "analytics", "all"], "description": "Business domain to inspect" }
    },
    "required": ["domain"]
  }
}
```

### 3. get_revenue
Revenue metrics from Stripe and database.

```json
{
  "name": "get_revenue",
  "description": "Returns current revenue metrics: MRR, subscriber count, tier breakdown, Stripe transaction totals.",
  "parameters": {
    "type": "object",
    "properties": {
      "period": { "type": "string", "enum": ["day", "week", "month", "all"], "description": "Time period for revenue data" }
    },
    "required": ["period"]
  }
}
```

### 4. get_system_health
Platform status check.

```json
{
  "name": "get_system_health",
  "description": "Returns platform status: deployment state, domain health (14 domains), database connection, API route count, recent errors.",
  "parameters": {
    "type": "object",
    "properties": {}
  }
}
```

### 5. get_nps
NPS survey results.

```json
{
  "name": "get_nps",
  "description": "Returns NPS survey results: score distribution, response count, trend over specified days.",
  "parameters": {
    "type": "object",
    "properties": {
      "days": { "type": "integer", "description": "Number of days to look back. Default 30." }
    }
  }
}
```

### 6. get_churn
Churn risk metrics.

```json
{
  "name": "get_churn",
  "description": "Returns churn metrics: at-risk accounts, engagement drops, last-activity dates.",
  "parameters": {
    "type": "object",
    "properties": {
      "days": { "type": "integer", "description": "Number of days of inactivity to flag. Default 30." }
    }
  }
}
```

### 7. create_task
Asana task creation.

```json
{
  "name": "create_task",
  "description": "Creates an Asana task in a specified project. Use when Tracy or Amy say 'add this to Asana' or 'create a task for X.'",
  "parameters": {
    "type": "object",
    "properties": {
      "title": { "type": "string", "description": "Task title" },
      "notes": { "type": "string", "description": "Task description/notes" },
      "assignee": { "type": "string", "description": "Email of assignee" },
      "due": { "type": "string", "description": "Due date in ISO format (YYYY-MM-DD)" },
      "project": { "type": "string", "description": "Asana project name or GID" }
    },
    "required": ["title"]
  }
}
```

### 8. publish_social
Social media publishing through proxy.

```json
{
  "name": "publish_social",
  "description": "Publishes a social media post to connected accounts. Goes through the social publisher proxy — validates content before posting.",
  "parameters": {
    "type": "object",
    "properties": {
      "content": { "type": "string", "description": "Post text content" },
      "platform": { "type": "string", "enum": ["facebook", "gbp"], "description": "Target platform" }
    },
    "required": ["content", "platform"]
  }
}
```

### 9. draft_review_response
Google review response drafting. Does NOT auto-post.

```json
{
  "name": "draft_review_response",
  "description": "Drafts a response to a Google review in Delta Dawn's voice. Returns the draft for human approval — does NOT auto-post.",
  "parameters": {
    "type": "object",
    "properties": {
      "reviewText": { "type": "string", "description": "The review text to respond to" },
      "rating": { "type": "number", "description": "Star rating (1-5)" }
    },
    "required": ["reviewText", "rating"]
  }
}
```

### 10. search_codebase
RAG search against codebase index.

```json
{
  "name": "search_codebase",
  "description": "Searches the HDI codebase index (4,392 files) using TF-IDF + pgvector. Returns relevant code snippets and file paths.",
  "parameters": {
    "type": "object",
    "properties": {
      "query": { "type": "string", "description": "Search query about the codebase" },
      "limit": { "type": "integer", "description": "Max results to return. Default 5." }
    },
    "required": ["query"]
  }
}
```

---

## Error Handling (All Tools)

Every tool returns structured JSON on error:

```json
{
  "error": true,
  "message": "Specific error description",
  "retryPossible": true
}
```

Gemini sees exact failure — no hallucination around errors.

---

## Auth: Agent-Bearer Pattern

All tools authenticate via:
- `Authorization: Bearer <AGENT_SECRET>` (short-lived JWT from Cloud Run)
- All actions logged to OpsActivity as `actor: delta_dawn`
- Tenant filtering enforced at the API layer, not the prompt layer

---

*Next: Grok Prompt 2 (NL2SQL design) → Prompt 3 (Multi-tenant system prompt) → Prompt 4 (Vertex config) → Prompt 5 (Safe write patterns)*
